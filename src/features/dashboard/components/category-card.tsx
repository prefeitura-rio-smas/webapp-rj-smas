...
export function CategoryCard({ title, description, buttons }: CategoryCardProps) {
  ...
  return (
    <div
      className="bg-white dark:bg-[#1a3b6d] rounded-md overflow-hidden transition-all duration-300 cursor-pointer relative h-auto md:h-64 shadow-md border border-[#c1cee3] dark:border-[#1a3b6d]"
      onMouseEnter={() => !isMobile && setIsHovered(true)}
      onMouseLeave={() => !isMobile && setIsHovered(false)}
      onClick={handleCardClick}
    >
      <div className="p-4 md:p-8 flex flex-col md:flex-row md:items-start gap-4 md:gap-6 h-full">
        <div className="rounded-full bg-white dark:bg-[#0a2756] p-4 transition-colors border border-[#c1cee3] dark:border-[#1a3b6d] mx-auto md:mx-0">
          {getIcon()}
        </div>
        <div className="flex-1 text-center md:text-left">
          <h2 className="text-[#0d2953] dark:text-white font-bold text-xl md:text-2xl mb-2 md:mb-3">{title}</h2>
          <p className="text-[#1f3b65] dark:text-gray-300 text-sm md:text-base">{description}</p>
        </div>
      </div>
      ...
      >
        <div className="flex flex-wrap justify-center gap-4 md:gap-8 p-4 md:p-6">
          {buttons.map((button) => (
            ...
            >
              <div className="bg-white dark:bg-[#0a2756] p-3 md:p-4 rounded-md mb-2 md:mb-3 transition-all duration-200 group-hover:border-[#00c2f7] dark:group-hover:bg-sky-700 group-active:border-[#00c2f7] dark:group-active:bg-sky-800 border border-[#c1cee3] dark:border-[#1a3b6d]">
                {getButtonIcon(button.icon)}
              </div>
              <span className="text-[#0d2953] dark:text-white text-sm md:text-base group-hover:text-[#00c2f7] dark:group-hover:text-sky-300">
                {button.title}
              </span>
              {!button.internal && (
                <span className="text-[#778cb1] dark:text-gray-400 text-xs mt-1">(link externo)</span>
              )}
            </button>
          ))}
        </div>
