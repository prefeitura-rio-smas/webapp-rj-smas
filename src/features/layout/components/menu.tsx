...
import { MenuItem } from "@/features/common/components/menu-item"
...
export function Menu({ setCurrentPage, sidebarVisible, setSidebarVisible }: MenuProps) {
  ...
  return (
    <aside
      className={`fixed md:relative z-50 w-full md:w-60 bg-white dark:bg-[#0a2756] border-r border-[#c1cee3] dark:border-[#1a3b6d] min-h-[calc(100vh-64px)] transition-all duration-300 ${sidebarVisible ? "left-0" : "-left-full md:left-0"}`}
    >
      <nav className="p-4">
        <div className="mb-6">
          <button
            onClick={() => handleMenuItemClick(() => setCurrentPage(null))}
            className="flex items-center gap-2 text-[#0d2953] dark:text-white p-3 rounded-md hover:bg-[#e0e7f2] dark:hover:bg-[#1a3b6d] w-full text-left text-base"
          >
        ...
        <div className="space-y-1">
          <h3 className="text-[#778cb1] dark:text-gray-400 text-sm font-medium px-2 mb-3">Serviços</h3>

          <Accordion type="multiple" className="w-full border-none">
            {menuItems.map((item) => (
              <AccordionItem key={item.id} value={item.id} className="border-none">
                <AccordionTrigger className="py-3 px-3 text-[#0d2953] dark:text-white hover:bg-[#e0e7f2] dark:hover:bg-[#1a3b6d] hover:no-underline rounded-md text-base">
                  <div className="flex items-center gap-2">
                    {item.icon}
                    <span>{item.title}</span>
                  </div>
                </AccordionTrigger>
                <AccordionContent className="pb-0 pt-1 bg-[#e0e7f2] dark:bg-[#0f3169] accordion-content">
                  <div className="space-y-1">
